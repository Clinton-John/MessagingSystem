...
// changing the user server settings to ensure the sessions only use the cookies
...
if (isset($_SESSION["user_id"])) {
    ...
    if (!isset($_SESSION["last_regeneration"])) { // checks if the session has generated an id 
        regenerate_session_id_loggedIn(); // generates the session id and sets it to the current time
    } else{
    $interval = 60*30;
       if ($_SESSION["last_regeneration"] >= $interval) { // if the last id is more than the 30 minutes then regenerate
